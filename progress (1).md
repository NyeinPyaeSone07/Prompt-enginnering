...
|---|---|---|---|---|
| **M0** | Scaffold & Layout — static `index.html`, no logic | COMPLETED (with deviation — see BUG-002) | *(teammate name)* | Sep 16, 2026 |
...
| **BUG-002** | Low | Delivered M0 `index.html` uses hand-written inline `<style>` CSS instead of the Tailwind CSS CDN tag, and does not yet include the Chart.js or SheetJS CDN `<script>` tags in `<head>`. This deviates from the locked stack in `prd.md` §Tech Constraints and `architecture.md` §2. | No console error — the page renders fine on its own CSS, so the deviation is silent unless checked against `architecture.md`. | OPEN | Not yet fixed. Before starting M1, add all three CDN `<script>` tags (Tailwind, Chart.js, SheetJS) to `<head>` per the team's decision to load them upfront in M0 even though unused until later milestones. Decide explicitly whether to keep the current hand-written CSS or migrate classes to Tailwind utilities — do not leave this ambiguous going into M1. |
